...
def predict_salary(position, relevance_score, selected):
    ...
    if selected:
        ...
        salary = base_salary + (max_salary - base_salary) * (relevance_score * 0.5)  
...
def process_file(file_path):
    try:
        ...
        if 'Category' not in df.columns and 'resume' not in df.columns:
            messagebox.showerror("Error", "The dataset must contain columns for 'Category' and 'resume'.")
            return

        category_column = 'Category' if 'Category' in df.columns else df.columns[0] 
        resume_column = 'resume' if 'resume' in df.columns else df.columns[1]  

        df['Extracted_skills'] = df[resume_column].apply(extract_skills_from_text)
